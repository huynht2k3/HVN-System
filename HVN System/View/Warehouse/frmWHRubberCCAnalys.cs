...
{
    public partial class frmWHRubberCCAnalys : Form
    {
        public frmWHRubberCCAnalys()
        {
            InitializeComponent();
        }
        ...
        {
            string strQry2 = "SELECT * FROM W_R_CCResult where cc_date=N'"+ cboCcDate.SelectedValue+ "'";
            string strQry = "select r_name,isnull(sum(sys_weight),0) as sys_weight,isnull(sum(cc_weight),0) as cc_weight,  \n ";
            strQry += "  count(sys_place) as sys_weight_box,count(cc_place) as cc_weight_box,count(sys_place)-count(cc_place) as gap_box, \n ";
            strQry += "  isnull(sum(sys_weight),0)-isnull(sum(cc_weight),0) as gap_weight \n ";
            strQry += "  from W_R_CCResult where cc_date=N'" + cboCcDate.SelectedValue + "' \n ";
            strQry += "  group by r_name \n ";
        ...
        {
            string strQry = "select cast(cc_date as varchar(12)) as cc_date from W_R_CCResult \n ";
        ...
        private void btnStockAdjustment_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            {
                frmWHCCAdjustment frm = new frmWHCCAdjustment(dt_Detail, cboCcDate.Text, "Rubber");
                frm.Show();
            }
            else
            {
        ...
        private void btnApprovalAdjustment_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            if (dt_Detail.Rows.Count > 0)
            {
                frmWHCCAdjustmentApprove frm = new frmWHCCAdjustmentApprove(cboCcDate.Text, "Rubber");
                frm.ShowDialog();
            }
            else
            {
