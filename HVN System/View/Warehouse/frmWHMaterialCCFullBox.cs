...
{
    public partial class frmWHMaterialCCFullBox : Form
    {
        public frmWHMaterialCCFullBox()
        {
            InitializeComponent();
        }
        public frmWHMaterialCCFullBox(string PIC,DateTime Cc_date)
        {
            InitializeComponent();
            txtPIC.Text = PIC;
        ...
        private void Add_Item(string QRCode)
        {
            ...
            if (string.IsNullOrEmpty(PIC))
            {
                ...
                strQry += " select N'"+dtpCCDate.Value.ToString("yyyy-MM-dd")+"',whmr_code,m_name,quantity \n ";
                strQry += " ,N'" + cboPlace.Text + "',N'" + txtPIC.Text.ToUpper() + "',getdate(),N'FULL BOX' \n ";
                strQry += " from W_M_ReceiveLabel where whmr_code=N'" + QRCode + "' \n ";
                conn.ExcuteQry(strQry);
            }
            else
            {
        ...
        private void Load_data(DateTime Cc_date,string QR_Code)
        {
            ...
            if (txtPIC.Text == "")
            {
                strQry += " where cc_date=N'" + dtpCCDate.Value.ToString("yyyy-MM-dd") + "' and m_kind=N'FULL BOX' \n ";
            }
            else
            {
                strQry += " where cc_date=N'" + dtpCCDate.Value.ToString("yyyy-MM-dd") + "' and m_kind=N'FULL BOX' and pic=N'" + txtPIC.Text.ToUpper() + "' \n ";
            }
            ...
            if (txtPIC.Text == "")
            {
                strQry2 += " where cc_date=N'" + dtpCCDate.Value.ToString("yyyy-MM-dd") + "' and m_kind=N'FULL BOX' \n ";
            }
            else
            {
                strQry2 += " where cc_date=N'" + dtpCCDate.Value.ToString("yyyy-MM-dd") + "' and m_kind=N'FULL BOX' and pic=N'" + txtPIC.Text.ToUpper() + "' \n ";
            }
            DataTable dt = conn.ExcuteDataTable(strQry2);
            dgvDetail.DataSource= dt;
            txtTotalBox.Text = dt.Rows.Count.ToString();
            string strQry3= "select m_name from W_M_ReceiveLabel where whmr_code=N'" + QR_Code + "' \n ";
